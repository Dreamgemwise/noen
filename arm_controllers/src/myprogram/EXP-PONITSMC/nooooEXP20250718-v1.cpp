...
namespace arm_controllers
{
  ...
  public:
    ...
    bool init(hardware_interface::EffortJointInterface* hw, ros::NodeHandle& n)
    {
      ...
      if (!n.getParam("lambda7_", lambda7_vector))
      {
        ROS_ERROR("Could not find lambda7_ parameter");
        return false;
      }
      if (lambda7_vector.size() != 2)
      {
        ROS_ERROR("lambda7_ parameter should have 2 elements");
        return false;
      }
      lambda7_ << lambda7_vector[0], lambda7_vector[1];

      if (!n.getParam("lambda8_", lambda8_vector))
      {
        ROS_ERROR("Could not find lambda8_ parameter");
        return false;
      }
      if (lambda8_vector.size() != 2)
      {
        ROS_ERROR("lambda8_ parameter should have 2 elements");
        return false;
      }
      lambda8_ << lambda8_vector[0], lambda8_vector[1];

      if (!n.getParam("lambda9_", lambda9_vector))
      {
        ROS_ERROR("Could not find lambda9_ parameter");
        return false;
      }
      if (lambda9_vector.size() != 2)
      {
        ROS_ERROR("lambda9_ parameter should have 2 elements");
        return false;
      }
      lambda9_ << lambda9_vector[0], lambda9_vector[1];

      if (!n.getParam("kke_", kke_vector))
      {
        ROS_ERROR("Could not find kke_ parameter");
        return false;
      }
      if (kke_vector.size() != 2)
      {
        ROS_ERROR("kke_ parameter should have 2 elements");
        return false;
      }
      kke_ << kke_vector[0], kke_vector[1];

      if (!n.getParam("epsilon_e_", epsilon_e_vector))
      {
        ROS_ERROR("Could not find epsilon_e_ parameter");
        return false;
      }
      if (epsilon_e_vector.size() != 2)
      {
        ROS_ERROR("epsilon_e_ parameter should have 2 elements");
        return false;
      }
      epsilon_e_ << epsilon_e_vector[0], epsilon_e_vector[1];
      ...
      //   return false;
      // }
      output_file_path_ = "/home/triyi/catkin_ws/src/arm_controllers/src/myprogram/EXP-PONITSMC/output_data.csv";
    ...
    void update(const ros::Time& time, const ros::Duration& period)
    {
      ...
      s_(0) = lambda7_(0) * e_(0) + lambda8_(0) * sign(e_(0)) * pow(fabs(e_(0)), 19.0 / 7.0) + lambda9_(0) * Part_of_Integral_(0);
      s_(1) = lambda7_(1) * e_(1) + lambda8_(1) * sign(e_(1)) * pow(fabs(e_(1)), 19.0 / 7.0) + lambda9_(1) * Part_of_Integral_(1);
      ...
      // tau_[0] = C_(0, 0) * dq_[0] + C_(0, 1) * dq_[4] + M_(0, 0) * (ddqd_(0) + (k1_(0) * Phi1_(0) - g_(0) + lambda9_(0) * pow(fabs(e_(0)), 1.0 / 3.0) * sign(e_(0))) / (lambda7_(0) + (19.0 / 7.0) * lambda8_(0) * pow(fabs(e_(0)), 12.0 / 7.0))) + M_(0, 1) * (ddqd_(1) + (k1_(1) * Phi1_(1) - g_(1) + lambda9_(1) * pow(fabs(e_(1)), 1.0 / 3.0) * sign(e_(1))) / (lambda7_(1) + (19.0 / 7.0) * lambda8_(1) * pow(fabs(e_(1)), 12.0 / 7.0)));
      // tau_[4] = C_(1, 0) * dq_[0] + C_(1, 1) * dq_[4] + M_(1, 0) * (ddqd_(0) + (k1_(0) * Phi1_(0) - g_(0) + lambda9_(0) * pow(fabs(e_(0)), 1.0 / 3.0) * sign(e_(0))) / (lambda7_(0) + (19.0 / 7.0) * lambda8_(0) * pow(fabs(e_(0)), 12.0 / 7.0))) + M_(1, 1) * (ddqd_(1) + (k1_(1) * Phi1_(1) - g_(1) + lambda9_(1) * pow(fabs(e_(1)), 1.0 / 3.0) * sign(e_(1))) / (lambda7_(1) + (19.0 / 7.0) * lambda8_(1) * pow(fabs(e_(1)), 12.0 / 7.0)));



      tau_[0] = C_(0, 0) * dq_[0] + C_(0, 1) * dq_[4] + M_(0, 0) * (ddqd_(0) + (kke_(0) * s_(0) + epsilon_e_(0) * sign(s_(0)) + lambda9_(0) * pow(fabs(e_(0)), 1.0 / 3.0) * sign(e_(0))) / (lambda7_(0) + (19.0 / 7.0) * lambda8_(0) * pow(fabs(e_(0)), 12.0 / 7.0))) + M_(0, 1) * (ddqd_(1) + (kke_(1) * s_(1) + epsilon_e_(1) * sign(s_(1)) + lambda9_(1) * pow(fabs(e_(1)), 1.0 / 3.0) * sign(e_(1))) / (lambda7_(1) + (19.0 / 7.0) * lambda8_(1) * pow(fabs(e_(1)), 12.0 / 7.0)));




      tau_[4] = C_(1, 0) * dq_[0] + C_(1, 1) * dq_[4] + M_(1, 0) * (ddqd_(0) + (kke_(0) * s_(0) + epsilon_e_(0) * sign(s_(0)) + lambda9_(0) * pow(fabs(e_(0)), 1.0 / 3.0) * sign(e_(0))) / (lambda7_(0) + (19.0 / 7.0) * lambda8_(0) * pow(fabs(e_(0)), 12.0 / 7.0))) + M_(1, 1) * (ddqd_(1) + (kke_(1) * s_(1) + epsilon_e_(1) * sign(s_(1)) + lambda9_(1) * pow(fabs(e_(1)), 1.0 / 3.0) * sign(e_(1))) / (lambda7_(1) + (19.0 / 7.0) * lambda8_(1) * pow(fabs(e_(1)), 12.0 / 7.0)));
      ...
      {
        std::cout
          // << std::setw(10) << lambda7_(0) << ", "
          //   << std::setw(10) << lambda7_(1) << ", "
          //   << std::setw(10) << lambda8_(0) << ", "
          //   << std::setw(10) << lambda8_(1) << ", "
          //   << std::setw(10) << lambda9_(0) << ", "
          //   << std::setw(10) << lambda9_(1) << ", "
          << std::setw(10) << dqr_(0) << ", "
          << std::setw(10) << dqr_(1) << ", "
          //   << std::setw(10) << ddqr_(0) << ", "
          //   << std::setw(10) << ddqr_(1) << ", "
  ...
  private:
    ...
    std::vector<double> lambda7_vector, lambda8_vector, lambda9_vector, fc_vector, fv_vector, fs_vector, kf_vector, wf_vector, km_vector, w1_vector, gamma1_vector, miu_vector, eta_vector, k1_vector, Md_vector, Bd_vector;
    std::vector<double> kke_vector, epsilon_e_vector;
    ...
    Eigen::Matrix<double, 2, 1> k2_, g_, dg_, Phi1_, Phi2_, Part_of_Integral_, dPart_of_Integral_, kke_, epsilon_e_;
    Eigen::Matrix<double, 2, 1> lambda7_, lambda8_, lambda9_, k3_, epsilon_, beta0_, qd_, dqd_, ddqd_, Bd_;
