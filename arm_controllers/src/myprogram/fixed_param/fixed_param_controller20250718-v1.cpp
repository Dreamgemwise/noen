...
#define PINOCCHIO_MODEL_DIR                                                    \
  "/home/triyi/catkin_ws/src/elfin_robot/elfin_description/urdf/elfin3.urdf"
#endif
    ...
    class AdmittanceController : public controller_interface::Controller<hardware_interface::EffortJointInterface> {
      public:
        ...
        bool init(hardware_interface::EffortJointInterface *hw, ros::NodeHandle &n) {
            ...
            Md_ << Md_vector[0], Md_vector[1], Md_vector[2], Md_vector[3];
            ...
            // dqd_ << 0.0,
            //   0.0;
            dqd_ << 0.0463754, -0.127923;
            // 0.0463754, -0.127923
            Part_of_Integral_ << 0.0, 0.0;
            // k1_ << 3.0,
            //   3.0;
            g_ << 0.0, 0.0;
            ...
            //   return false;
            // }
            output_file_path_ = "/home/triyi/catkin_ws/src/arm_controllers/src/myprogram/fixed_param/output_data.csv";
            ...
            // 写入文件头
            data_file_ << "Time,e_0,e_1,s_0,s_1,ds_0,ds_1,k1_0,k1_1,q_0,q_4,dq_0,dq_4,dqr_0,dqr_1,dqd_0,dqd_1,ddqd_0,ddqd_1,tau_0,tau_4,Fext_0,Fext_1" << std::endl;
        ...
        void commandCB(const std_msgs::Float64MultiArrayConstPtr &msg) {
            ...
            for (unsigned int i = 0; i < n_joints; i++) q_cmd_(i) = msg->data[i];
        }
        ...
        //     {
        //       for (size_t i = 0; i < 6; ++i)
        //         f_offset_[i] = f_offset_acc_[i] /
        //         static_cast<double>(FT_OFFSET_SAMPLES);
        ...
        void updateFTsensor(const geometry_msgs::WrenchStamped::ConstPtr &msg) {
            /* ---------- 原始读数 ---------- */
            const auto &w = msg->wrench;
            const double raw[6] = { w.force.x, w.force.y, w.force.z, w.torque.x, w.torque.y, w.torque.z };
            ...
            if (!ft_offset_ready_) {
                ...
                if (++ft_offset_cnt_ >= FT_OFFSET_SAMPLES) {
                    for (size_t i = 0; i < 6; ++i) f_offset_[i] = f_offset_acc_[i] / static_cast<double>(FT_OFFSET_SAMPLES);
                    ft_offset_ready_ = true;
                    ROS_INFO_STREAM("[FT] Zero-offset calibrated (samples = " << FT_OFFSET_SAMPLES << ")");
                }
                return;  // 标定期不向下传递数据
            }
        ...
        Eigen::Matrix<double, 2, 1> get_dqr(double q1, double q2, double l1, double l2, double xc, double yc, double r0, double k0, double epsilon1, double VD, double c0, Eigen::Matrix<double, 2, 2> J_INV_) {
            // 1、定义变量
            double x, y, u, k, c;
            ...
            k = k0 / (sqrt(du_(0) * du_(0) + du_(1) * du_(1)) + epsilon1);
        ...
        Eigen::Matrix<double, 2, 1> get_dk1(double s1, double s2, double k1_1, double k1_2, Eigen::Matrix<double, 2, 1> km_, Eigen::Matrix<double, 2, 1> w1_, Eigen::Matrix<double, 2, 1> gamma1_, Eigen::Matrix<double, 2, 1> eta_, Eigen::Matrix<double, 2, 1> miu_) {
            // 1、定义变量
        ...
        void update(const ros::Time &time, const ros::Duration &period) {
            ...
            // 导纳模型计算ddqd
            // ddqd_(0) = Md_INV_(0, 0) * (F_Joint_(0) - Bd_(0) * (dqd_(0) - dqr_(0))) +
            // Md_INV_(0, 1) * (F_Joint_(1) - Bd_(1) * (dqd_(1) - dqr_(1))) + ddqr_(0);
            // ddqd_(1) = Md_INV_(1, 0) * (F_Joint_(0) - Bd_(0) * (dqd_(0) - dqr_(0))) +
            // Md_INV_(1, 1) * (F_Joint_(1) - Bd_(1) * (dqd_(1) - dqr_(1))) + ddqr_(1);
            // ddqd_(0) = Md_INV_(0, 0) * (F_Joint_(0) - Bd_(0) * (dqd_(0) - dqr_(0))) +
            // ddqr_(0); ddqd_(1) = Md_INV_(1, 1) * (F_Joint_(1) - Bd_(1) * (dqd_(1) -
            // dqr_(1))) + ddqr_(1);
            ...
            // dqd_(0) += ddqd_(0) * T;
            // dqd_(1) += ddqd_(1) * T;
            // dqd_(0) = 1.0/Bd_(0) * (F_Joint_(0) - Md_(0, 0) * (ddqd_(0) - ddqr_(0)))
            // + dqr_(0); dqd_(1) = 1.0/Bd_(1) * (F_Joint_(1) - Md_(1, 1) * (ddqd_(1) -
            // ddqr_(1))) + dqr_(1);
            ...
            de_(0) = (e_(0) - e_last_(0)) / T;
            de_(1) = (e_(1) - e_last_(1)) / T;
            e_last_(0) = e_(0);
            e_last_(1) = e_(1);
            ...
            M_ << data.M(0, 0), data.M(0, 4), data.M(4, 0), data.M(4, 4);
            pinocchio::computeCoriolisMatrix(model, data, q_m_, dq_m_);
            C_ << data.C(0, 0), data.C(0, 4), data.C(4, 0), data.C(4, 4);
            ...
            s_(0) = lambda1_(0) * e_(0) + lambda2_(0) * sign(e_(0)) * pow(fabs(e_(0)), 19.0 / 7.0) + lambda3_(0) * Part_of_Integral_(0);
            s_(1) = lambda1_(1) * e_(1) + lambda2_(1) * sign(e_(1)) * pow(fabs(e_(1)), 19.0 / 7.0) + lambda3_(1) * Part_of_Integral_(1);
            ds_(0) = lambda1_(0) * de_(0) + (12.0 / 7.0) * lambda2_(0) * pow(fabs(e_(0)), 12.0 / 7.0) * de_(0) + lambda3_(0) * pow(fabs(e_(0)), 1.0 / 3.0) * sign(e_(0));
            ds_(1) = lambda1_(1) * de_(1) + (12.0 / 7.0) * lambda2_(1) * pow(fabs(e_(1)), 12.0 / 7.0) * de_(1) + lambda3_(1) * pow(fabs(e_(1)), 1.0 / 3.0) * sign(e_(1));
            ...
            g_(0) += dg_(0) * T;
            g_(1) += dg_(1) * T;
            tau_[0] = C_(0, 0) * dq_[0] + C_(0, 1) * dq_[4] + M_(0, 0) * (ddqd_(0) + (k1_(0) * Phi1_(0) - g_(0) + lambda3_(0) * pow(fabs(e_(0)), 1.0 / 3.0) * sign(e_(0))) / (lambda1_(0) + (19.0 / 7.0) * lambda2_(0) * pow(fabs(e_(0)), 12.0 / 7.0))) + M_(0, 1) * (ddqd_(1) + (k1_(1) * Phi1_(1) - g_(1) + lambda3_(1) * pow(fabs(e_(1)), 1.0 / 3.0) * sign(e_(1))) / (lambda1_(1) + (19.0 / 7.0) * lambda2_(1) * pow(fabs(e_(1)), 12.0 / 7.0)));
            tau_[4] = C_(1, 0) * dq_[0] + C_(1, 1) * dq_[4] + M_(1, 0) * (ddqd_(0) + (k1_(0) * Phi1_(0) - g_(0) + lambda3_(0) * pow(fabs(e_(0)), 1.0 / 3.0) * sign(e_(0))) / (lambda1_(0) + (19.0 / 7.0) * lambda2_(0) * pow(fabs(e_(0)), 12.0 / 7.0))) + M_(1, 1) * (ddqd_(1) + (k1_(1) * Phi1_(1) - g_(1) + lambda3_(1) * pow(fabs(e_(1)), 1.0 / 3.0) * sign(e_(1))) / (lambda1_(1) + (19.0 / 7.0) * lambda2_(1) * pow(fabs(e_(1)), 12.0 / 7.0)));
            ...
            // 不同的轴分开：轴0到轴6
            if (fabs(q_[0] - q_init_[0]) > delta) { tau1_[0] = (-1) * be_[0] * tanh(B * dq_[0]) - ke_[0] * tanh(K * ((q_[0] - q_init_[0]) - delta)); }  // 轴0
            if (fabs(q_[1] - q_init_[1]) > delta) { tau1_[1] = (-1) * be_[1] * tanh(B * dq_[1]) - ke_[1] * tanh(K * ((q_[1] - q_init_[1]) - delta)); }  // 轴1
            if (fabs(q_[2] - q_init_[2]) > delta) { tau1_[2] = (-1) * be_[2] * tanh(B * dq_[2]) - ke_[2] * tanh(K * ((q_[2] - q_init_[2]) - delta)); }  // 轴2
            if (fabs(q_[3] - q_init_[3]) > delta) { tau1_[3] = (-1) * be_[3] * tanh(B * dq_[3]) - ke_[3] * tanh(K * ((q_[3] - q_init_[3]) - delta)); }  // 轴3
            if (fabs(q_[4] - q_init_[4]) > delta) { tau1_[4] = (-1) * be_[4] * tanh(B * dq_[4]) - ke_[4] * tanh(K * ((q_[4] - q_init_[4]) - delta)); }  // 轴4
            if (fabs(q_[5] - q_init_[5]) > delta) { tau1_[5] = (-1) * be_[5] * tanh(B * dq_[5]) - ke_[5] * tanh(K * ((q_[5] - q_init_[5]) - delta)); }  // 轴5
            // 加入到控制率tau中即可
            // 再加上重力补偿项
            tau_[1] = tau1_[1] + G_(1);
            tau_[2] = tau1_[2] + G_(2);
            ...
            tau_cmd_ = tau_;
            // 对扭矩命令进行限幅
            for (size_t i = 0; i < n_joints; i++) { tau_cmd_[i] = std::max(min_val_[i], std::min(max_val_[i], tau_cmd_[i])); }
            for (size_t i = 0; i < n_joints; i++) { joints_[i].setCommand(tau_cmd_[i]); }
            ...
            if (count < 10) {
                ...
                std::cout << std::setw(10) << dqr_(0) << ", " << std::setw(10) << dqr_(1) << ", " << std::setw(10) << dqd_(0) << ", " << std::setw(10) << dqd_(1) << ", " << std::setw(10) << Fext_(0) << ", " << std::setw(10) << Fext_(1) << std::endl;  // 换行
                count = 0;
            }
            ...
            if (t <= 20.0) {
                // 将数据写入文件
                data_file_ << t << ", " << e_(0) << ", " << e_(1) << ", " << s_(0) << ", " << s_(1) << ", " << ds_(0) << ", " << ds_(1) << ", " << k1_(0) << ", " << k1_(1) << ", " << q_[0] << ", " << q_[4] << ", " << dq_[0] << ", " << dq_[4] << ", " << dqr_(0) << ", " << dqr_(1) << ", " << dqd_(0) << ", " << dqd_(1) << ", " << ddqd_(0) << ", " << ddqd_(1) << ", " << tau_cmd_[0] << ", " << tau_cmd_[4] << ", " << Fext_(0) << ", " << Fext_(1) << std::endl;
                data_file_.flush();
                //  data_file_ << "Time,e_0,e_1,s_0,s_1,ds_0,ds_1,k1_0,k1_1,q_0,q_4,dq_0,dq_4,dqr_0,dqr_1,dqd_0,dqd_1,ddqd_0,ddqd_1,tau_0,tau_4,Fext_0,Fext_1" << std::endl;
      ...
      private:
        ...
        std::vector<double> lambda1_vector, lambda2_vector, lambda3_vector, fc_vector, fv_vector, fs_vector, kf_vector, wf_vector, km_vector, w1_vector, gamma1_vector, miu_vector, eta_vector, k1_vector, Md_vector, Bd_vector;
        ...
        Eigen::Matrix<double, 2, 1> F_Joint_, Fext_, dqr_, ddqr_, dqr_last_, fc_, fv_, fs_, kf_, wf_, de_, e_last_, ds_;
